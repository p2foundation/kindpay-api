...
import { ONE4ALL_APIKEY, ONE4ALL_APISECRET, ONE4ALL_BASEURL } from 'src/config';
...
export class SmsService {
  ...
  public SendSMS(transDto: SmsDto): Observable<AxiosResponse<SmsDto>> {
    ...
    const s2Params = {
      ...
      message: message || '',
      sender_id: senderId || '',
      trxn: this.defaultUtility.generateTransactionId() || '',
    };
    ...
    return this.httpService
      ...
      .pipe(
        map((pbsRes) => {
          this.logger.verbose(
            `BulkSMS response ++++ ${JSON.stringify(pbsRes.data)}`,
          );
          return pbsRes.data;
        }),
        catchError((taError) => {
          this.logger.error(
            `BulkSMS ERROR response ---- ${JSON.stringify(taError.data)}`,
          );
          return taError.data;
        }),
      );
